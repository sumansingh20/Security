...
export default function ExamAttemptPage() {
  ...
  const loadExamSession = async (token: string) => {
    try {
      const response = await fetch(`${process.env.NEXT_PUBLIC_API_URL}/exam-engine/session/${token}`, {
        headers: {
    ...
    heartbeatIntervalRef.current = setInterval(async () => {
      ...
      try {
        const response = await fetch(
          `${process.env.NEXT_PUBLIC_API_URL}/exam-engine/session/${currentToken}/heartbeat`,
          {
            method: 'POST',
            headers: {
        ...
        setViolationCount(data.violationCount || violationCount);
      } catch (err) {
  ...
  const reportViolation = async (type: string, details: string) => {
    ...
    try {
      const response = await fetch(
        `${process.env.NEXT_PUBLIC_API_URL}/exam-engine/session/${sessionToken}/violation`,
        {
          method: 'POST',
          headers: {
            'Content-Type': 'application/json',
  ...
  const saveAnswer = async (questionId: string, answer: Answer) => {
    ...
    try {
      const response = await fetch(
        `${process.env.NEXT_PUBLIC_API_URL}/exam-engine/session/${sessionToken}/answer`,
        {
          method: 'POST',
          headers: {
            'Content-Type': 'application/json',
  ...
  const handleSubmit = async () => {
    ...
    try {
      const response = await fetch(
        `${process.env.NEXT_PUBLIC_API_URL}/exam-engine/session/${sessionToken}/submit`,
        {
          method: 'POST',
          headers: {
            'Content-Type': 'application/json',
  ...
  const handleTimeExpired = async () => {
    ...
    if (currentToken && currentFingerprint) {
      try {
        await fetch(
          `${process.env.NEXT_PUBLIC_API_URL}/exam-engine/session/${currentToken}/submit`,
          {
            method: 'POST',
            headers: {
              'Content-Type': 'application/json',
