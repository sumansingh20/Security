...
export const getDashboardStats = async (req, res, next) => {
  try {
    ...
    const results = await Promise.allSettled([
      ...
      Violation.countDocuments({ createdAt: { $gte: last24Hours } }),
      Exam.find().sort({ createdAt: -1 }).limit(10).select('title subject status startTime endTime').lean(),
      Submission.find()
        .sort({ submittedAt: -1 })
        .limit(10)
    ...
    const totalExams = results[0].status === 'fulfilled' ? results[0].value : 0;
    const activeExams = results[1].status === 'fulfilled' ? results[1].value : 0;
    const publishedExams = results[2].status === 'fulfilled' ? results[2].value : 0;
    const totalStudents = results[3].status === 'fulfilled' ? results[3].value : 0;
    const totalSubmissions = results[4].status === 'fulfilled' ? results[4].value : 0;
    const recentViolations = results[5].status === 'fulfilled' ? results[5].value : 0;
    const recentExams = results[6].status === 'fulfilled' ? (results[6].value || []) : [];
    const recentSubmissions = results[7].status === 'fulfilled' ? (results[7].value || []) : [];

    // Calculate average score from recent submissions (with null safety)
    let averageScore = 0;
    const scoredSubmissions = (recentSubmissions || []).filter(s => s && s.totalMarks > 0);
    if (scoredSubmissions.length > 0) {
      const totalScore = scoredSubmissions.reduce((sum, s) => sum + ((s.marksObtained / s.totalMarks) * 100), 0);
      averageScore = Math.round(totalScore / scoredSubmissions.length);
    }
