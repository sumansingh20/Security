...
export const questionSchemas = {
  create: Joi.object({
    ...
    questionType: Joi.string().valid('mcq-single', 'mcq-multiple', 'true-false').default('mcq-single'),
    ...
    correctOptions: Joi.array().items(Joi.number().min(0)).min(1).required(),
    imageUrl: Joi.string().uri().allow(null, ''),
  ...
  update: Joi.object({
    ...
    questionType: Joi.string().valid('mcq-single', 'mcq-multiple', 'true-false'),
    options: Joi.array().items(
      Joi.object({
        _id: Joi.string(),
        text: Joi.string().required(),
